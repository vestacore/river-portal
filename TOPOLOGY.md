...
packages/
  foundation/       identity, language, privacy          (layer 0)
    kernel  i18n  privacy
...
2. **Dependencies point down or sideways, never up.** foundation ← record ← river/assist ← surface ← compose ← apps. There are no cycles. Every cross-package import is declared in `package.json`.
...
| `@river/log` | record (1) | `packages/record/log` | Append-only event log: event envelope, commit with projectors. | kernel, store, privacy | — |
...
| `@river/flows` | river (2) | `packages/river/flows` | Flows linking gifts to needs: dispatch, costs, delivery, confirmation, gratitude. | kernel, i18n, privacy, store, log, needs, gifts | — |
| `@river/gifts` | river (2) | `packages/river/gifts` | Campaigns, offers and gifts. | i18n, kernel, log, privacy, store | — |
| `@river/needs` | river (2) | `packages/river/needs` | Needs: intake, triage, status and recipient tracking. | kernel, i18n, privacy, store, log | — |
...
| `@river/reports` | surface (3) | `packages/surface/reports` | Reports generated from flows, edited in place, published with consent checks. | kernel, i18n, privacy, store, log, needs, gifts, flows, content | — |
| `@river/runtime` | compose (4) | `packages/compose/runtime` | Wires store driver, projectors and demo seed for applications. | kernel, i18n, privacy, store, log, needs, gifts, flows, assist, content, reports, feed, pages | — |
| `@river/web` | apps (5) | `apps/web` |  | content, feed, flows, gifts, i18n, kernel, log, needs, pages, privacy, reports, runtime | @tiptap/core, @tiptap/pm, @tiptap/react, @tiptap/starter-kit, next, react, react-dom, tailwindcss, @tailwindcss/postcss, @types/react, @types/react-dom |
