...
type Kind = 'money' | 'goods' | 'transport' | 'service' | 'time';
type Props = {
  t: Dictionary['give'];
  kinds: Kind[];
  amounts: Array<{ value: string; label: string }>;
  currency: string;
  instructions: string;
...
const icons = { money: 'coins', goods: 'basket', transport: 'van', service: 'hand', time: 'heart' } as const;

/** The giving form: what, how much or what exactly, towards which campaign, and who you are. */
export function GiveForm({ t, kinds, amounts, currency, instructions, campaigns, campaignId, initialKind }: Props) {
  const [state, action, pending] = useActionState<GiveState, FormData>(pledgeAction, { done: false, errors: {}, values: {} });
  const [kind, setKind] = useState(state.values.kind ?? initialKind ?? kinds[0] ?? 'money');
  const [amount, setAmount] = useState(state.values.amount ?? amounts[1]?.value ?? amounts[0]?.value ?? '');
  ...
  if (state.done) {
    return (
      <div className="chamfer animate-rise p-8 text-center [--cut:24px] [--fill:var(--color-sunrise-100)]">
        ...
        {instructions ? <p className="mx-auto mt-3 max-w-md text-ink-700">{instructions}</p> : null}
      </div>
    );
  }
  return (
    <form action={action} className="space-y-7" noValidate onChange={(e) => { const el = e.target as HTMLInputElement; if (el.name === 'kind') setKind(el.value); }}>
      ...
      <Field label={t.kind} error={err('kind')}>
        <ChoiceCards name="kind" columns={3} defaultValue={kind} options={kinds.map((k) => ({ value: k, label: t.kinds[k], icon: icons[k] }))} />
      </Field>
      {kind === 'money' ? (
        <Field label={t.amount.replace('{currency}', currency)} error={err('amount')} htmlFor="amount">
          <div className="flex flex-wrap gap-2">
            {amounts.map((a) => (
              <button key={a.value} type="button" onClick={() => setAmount(a.value)} aria-pressed={amount === a.value} className={`px-5 py-2.5 font-semibold tabular-nums ring-1 transition ${amount === a.value ? 'bg-ink-900 text-paper ring-ink-900' : 'bg-white text-ink-900 ring-graphite/30 hover:ring-graphite'}`}>{a.label}</button>
            ))}
