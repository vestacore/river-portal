...
export function Hero({ locale, dict, blocks }: { locale: Locale; dict: Dictionary; blocks: BlockMap }) {
  return (
        ...
        <div className="grid items-center gap-12 lg:grid-cols-[1fr_1.1fr] lg:gap-10">
          <div className="animate-rise">
            <p className="annot mb-8 flex items-center gap-3 text-ink-500">
              <span className="text-sunrise-600">{dict.home.sheet}</span>
              <span className="h-px w-16 bg-graphite/40" aria-hidden="true" />
            </p>
            <Editable blocks={blocks} blockId="home.hero.title" locale={locale} as="h1" className="text-balance text-[2.6rem] font-semibold leading-[1.04] tracking-[-0.03em] text-ink-900 sm:text-6xl lg:text-[3.5rem] xl:text-6xl" />
            <Editable blocks={blocks} blockId="home.hero.lead" locale={locale} className="mt-6 max-w-xl text-lg text-ink-700 sm:text-xl" />
