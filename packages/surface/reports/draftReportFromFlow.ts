...
/** Generates a private report from a flow's records, in every locale, ready to edit in place. */
export async function draftReportFromFlow(env: CommandEnv, input: { flowId: string }): Promise<string> {
  const facts = await collectReportFacts(env.store, env.ctx.orgId, input.flowId);
