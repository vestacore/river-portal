...
export async function collectReportFacts(store: DocStore, orgId: string, flowId: string): Promise<ReportFacts> {
  ...
  for (const cost of flow.costs.filter((c) => c.status === 'approved')) {
    costBreakdown[cost.kind] = (costBreakdown[cost.kind] ?? 0) + cost.gbpMinor;
    costsGbpMinor += cost.gbpMinor;
  }
  ...
  return {
    ...
    moneyGbpMinor: gifts.reduce((sum, g) => sum + (g.currency === 'GBP' && g.amountMinor ? g.amountMinor : 0), 0),
    costsGbpMinor,
    costBreakdown,
