      ...
      <section>
        ...
        {suggested.length === 0 ? <p className="text-ink-500">{dict.common.none}</p> : (
          <ul className="grid gap-4">
            {suggested.map((item) => (
              <li key={item.id}>
                <form action={publishFeedItemAction} className="sketch bg-paper p-5">
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <label className="text-sm font-semibold">English (UK)<textarea name="textEn" rows={4} readOnly={!canEdit} defaultValue={item.text['en-GB']} className={`${inputClass} mt-1 font-normal`} /></label>
                    <label className="text-sm font-semibold">Українська<textarea name="textUk" lang="uk" rows={4} readOnly={!canEdit} defaultValue={item.text.uk} className={`${inputClass} mt-1 font-normal`} /></label>
                  </div>
        ...
        <ul className="grid gap-3">
          {published.map((item) => (
            <li key={item.id} className="flex items-start gap-3 border-t border-graphite/60 pt-3">
              ...
              {canEdit ? <form action={withdrawFeedItemAction}><input type="hidden" name="itemId" value={item.id} /><button className="border border-attention-500/40 px-3 py-1 text-xs font-semibold text-attention-500">{t.withdraw}</button></form> : null}
            </li>
          ))}
        </ul>
      </section>
