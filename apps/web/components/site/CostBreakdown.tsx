...
export function CostBreakdown({ breakdown, currency, locale, dict }: { breakdown: Partial<Record<string, number>>; currency: string; locale: Locale; dict: Dictionary }) {
  ...
  return (
    <ul className="space-y-3">
      {rows.map(([kind, minor]) => (
        <li key={kind}>
          <div className="mb-1 flex justify-between text-sm"><span className="font-medium text-ink-900">{(dict.costKinds as Record<string, string>)[kind] ?? kind}</span><span className="tabular-nums text-ink-700">{formatMoney(minor ?? 0, currency, locale)}</span></div>
