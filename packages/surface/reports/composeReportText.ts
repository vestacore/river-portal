...
export function composeReportText(facts: ReportFacts, locale: Locale): { title: string; paragraphs: string[] } {
  ...
  const costs = Object.entries(facts.costBreakdown)
    .map(([kind, minor]) => `${costLabels[kind]?.[locale] ?? kind} ${formatMoney(minor ?? 0, 'GBP', locale)}`)
    .join(', ');
  ...
  if (locale === 'uk') {
    ...
    paragraphs.push(`У цій доставці — ${gifts}${facts.moneyGbpMinor ? `, з них грошима ${formatMoney(facts.moneyGbpMinor, 'GBP', locale)}` : ''}.`);
    if (costs) paragraphs.push(`Витрати на доставку — ${formatMoney(facts.costsGbpMinor, 'GBP', locale)} (${costs}). Ми показуємо їх відкрито, бо чесні цифри — частина дару.`);
  ...
  paragraphs.push(`${facts.giftsCount} ${facts.giftsCount === 1 ? 'gift' : 'gifts'} flowed into this delivery${facts.moneyGbpMinor ? `, including ${formatMoney(facts.moneyGbpMinor, 'GBP', locale)} in money` : ''}.`);
  if (costs) paragraphs.push(`Delivery costs came to ${formatMoney(facts.costsGbpMinor, 'GBP', locale)} (${costs}). We show them openly, because honest numbers are part of the gift.`);
